...
## 🧑‍🤝‍🧑 역할 (Roles)
<!-- 각 시스템 / 클래스 / 담당자별 역할 정의 -->

| 역할 | 설명 |
|------|------|
| **Player** | 외형이 동일한 군중 속에서 적을 찾아 제거하는 플레이어 캐릭터 |
| **AI (NPC)** | 플레이어처럼 행동하며 혼란을 유발하는 NPC |
| **GameMode/GameState** | 스테이지 시작, 스폰 관리, 승패 판정, 순위 계산 담당 |
| **GAS (Gameplay Ability System)** | 공격 판정, 데미지 계산, 어빌리티 쿨타임 등 관리 |
| **UIManager** | 생존자 수, 타이머, 순위, 결과 UI 표시 |

---

## ⚙️ 주요 기능 (Core Features)
<!-- 구현한 주요 기능 및 시스템을 항목별로 정리 -->

### 🎯 전투 및 생존
- ✅ **GAS 기반 전투 시스템** - Gameplay Ability System을 활용한 어빌리티 관리
- ✅ **실시간 생존자 수 HUD** - 남은 생존자 수 실시간 표시
- ✅ **체력 및 사망 판정** - AttributeSet 기반 체력 관리 및 사망 처리
- ✅ **순위 시스템** - 사망 순서 기반 실시간 순위 계산

### 🧠 AI 시스템
- ✅ **BehaviorTree 기반 AI** - AI가 플레이어처럼 자연스럽게 행동
- ✅ **랜덤 행동 패턴** - 목표 탐색, 시선 회피, 순찰 로직 구현
